    ...
    private var developerSection: some View {
        Section("tamerlan swift") {
            VStack(alignment: .leading) {
                ...
                Text("Приложение разработано Тамерланом Губайдуллиным как пет-проект для резюме. Оно использует SwiftUI и написано на Swift. В проекте используется многопоточность, publisher/subsribers и сохранение данных 😉")
                    .font(.headline)
                  //  .fontWeight(.medium)
                    .foregroundStyle(Color.theme.accent)
            }
